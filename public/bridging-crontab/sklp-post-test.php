...
// post sklp 
  ...
  //echo $response_post_sklp["result"];
  
  $post_1 = json_decode($response_post_sklp);
  
  echo "result response with parsing :";
  echo $post_1->result;
  echo "<br>";
  echo "<br>";
  echo "<br>";
  echo "after post sklp : ";
  echo "</br>";
  var_dump($post_1);
  echo "<br>";
  echo "<br>";

  if (isset($post_1->result)){
    $submit='
            {"jsonrpc": "2.0",
              "params": {
...
//                   and a.index_load > (select ifnull(max(ref),0) from SKLP_API_Log where task_code=\'' . $schedule->number . '\') 
