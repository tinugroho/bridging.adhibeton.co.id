...
if (!empty($result['databsp'])) {
	...
	foreach ($result['databsp'] as $jo) {
		...
		$query = "select a.Ticket_Id, a.index_load , Createdate as RecordDate, a.Qty_Jobmix as Load_Size, d.Jobmix_Code, a.BP_ID, a.CreateBy, b.api, b.api_time_correction, b.bp_name, c.region_name
				...
				where a.Remarks = '" . $jo['JO_Number'] . "' " . $last_post_id . $apiupdate . " and b.api='dev-bspi' order by index_load asc limit 5";
		...
		while ($load = mysqli_fetch_array($arrLoadJoNumber)) {
			...
			if ($material != '' && $kirim == true) {
				...
				$url = "http://192.168.100.17/apberpdev/erp/eaccounting/tools/api/sfservice.cfc?method=setDataBSPI&JO_Number=" . $jo['JO_Number'] . "&Reference=A" . $load['index_load'] . "&Trx_Date=" . $RecordDate->format("Y-m-d") . "&Ship_Distance=1&Output_Qty=" . $load['Load_Size'] . "&Input_Qty=" . $material_akumulasi;
				...
				if ($result['return'] == 1) {
					...
					if ($first) {
						$query = "INSERT INTO `API_Logs_Detail`(`type`, `Method`, `JO_Number`, `Jobmix_ERP`, `Req_Qty`, `Remain_Qty`, `CreateByERP`, `CreateDateERP`, `UpdateByERP`, `UpdateDateERP`, 
									 `Keterangan`, bp_id) VALUES ('dev-bspi', 'GET', '" . $jo['JO_Number'] . "', '" . $jo['Item_Code'] . "', " . $jo['Item_Qty'] . ", " . $jo['Remaining_Qty'] . ", '" . $jo['Created_By'] . "', 
					...
					$query = "INSERT INTO `API_Logs_Detail`(`type`, `Method`, `JO_Number`, `Jobmix_ERP`, `Jobmix_BP`, `Req_Qty`, `Remain_Qty`, `Post_Qty`, `BP`, `CreateByERP`, `CreateDateERP`, `UpdateByERP`, `UpdateDateERP`, CreateByBP, CreateDateBP, Regional,
								`Material`, `Keterangan`, `index_load`, Ticket_Id, bp_id) VALUES ('dev-bspi', 'POST', '" . $jo['JO_Number'] . "', '" . $jo['Item_Code'] . "', '" . $load['Jobmix_Code'] . "', " . $jo['Item_Qty'] . ", " . ($jo['Remaining_Qty'] - $total_qty_load) . ", " . $load['Load_Size'] . ", '" . $load['bp_name'] . "', '" . $jo['Created_By'] . "', 
					...
					$query = "	INSERT INTO `API_Post_Gagal`(`type`, `JO_Number`, `Jobmix_ERP`, `Jobmix_BP`, `index_load`, `Load_Size`, `Material`, `Keterangan`, `RecordDate`, `jml_post`, Ticket_Id, bp_id) 
									VALUES ('dev-bspi', '" . $jo['JO_Number'] . "', '" . $jo['Item_Code'] . "', '" . $load['Jobmix_Code'] . "', " . $load['index_load'] . ", " . $load['Load_Size'] . ", '" . $material . "', '" . $result['return'] . "', '" . $RecordDate->format("Y-m-d H:i:s") . "', 1," . $load['Ticket_Id'] . "," . $load['BP_ID'] . ")";
					mysqli_query($con_ab, $query);
				}
				sleep(1);
			} else {
				...
				$query = "	INSERT INTO `API_Post_Gagal`(`type`, `JO_Number`, `Jobmix_ERP`, `Jobmix_BP`, `index_load`, `Load_Size`, `Material`, `Keterangan`, `RecordDate`, `jml_post`,Ticket_Id, bp_id) 
								VALUES ('dev-bspi', '" . $jo['JO_Number'] . "', '" . $jo['Item_Code'] . "', '" . $load['Jobmix_Code'] . "', " . $load['index_load'] . ", " . $load['Load_Size'] . ", '" . $material . "', '" . $invalid . "', '" . $RecordDate->format("Y-m-d H:i:s") . "', 1," . $load['Ticket_Id'] . "," . $load['BP_ID'] . ")";
				mysqli_query($con_ab, $query);
			}
		}
