...
if (!empty($result['databsp'])) {
	...
	foreach ($result['databsp'] as $jo) {
		...
		$query = "select max(a.index_load) as index_load ,a.LoadID, max(a.RecordDate) as RecordDate, a.Load_Size, a.Item_Code, a.BP_ID, a.CreatedBy, b.api, b.api_time_correction, b.bp_name, c.region_name
				...
				where a.Delivery_Instruction = '" . $jo['JO_Number'] . "' " . $last_post_id . $apiupdate . " and a.Ticket_Status='O' and b.api='live-bspi' group by LoadID order by index_load asc limit 5";
		...
		while ($load = mysqli_fetch_array($arrLoadJoNumber)) {
			...
			if ($material != '' && $kirim == true) {
				...
				$url = "http://192.168.100.17/apberp/erp/eaccounting/tools/api/sfservice.cfc?method=setDataBSPI&JO_Number=" . $jo['JO_Number'] . "&Reference=" . $load['index_load'] . "&Trx_Date=" . $RecordDate->format("Y-m-d") . "&Ship_Distance=1&Output_Qty=" . $load['Load_Size'] . "&Input_Qty=" . $material_akumulasi;
				...
				if ($result['return'] == 1) {
					...
					$query = "INSERT INTO `API_Logs_Detail`(`type`, `Method`, `JO_Number`, `Jobmix_ERP`, `Req_Qty`, `Remain_Qty`, `CreateByERP`, `CreateDateERP`, `UpdateByERP`, `UpdateDateERP`, 
								`Keterangan`) VALUES ('live-bspi', 'GET', '" . $jo['JO_Number'] . "', '" . $jo['Item_Code'] . "', " . $jo['Item_Qty'] . ", " . $jo['Remaining_Qty'] . ", '" . $jo['Created_By'] . "', 
					...
					$query = "INSERT INTO `API_Logs_Detail`(`type`, `Method`, `JO_Number`, `Jobmix_ERP`, `Jobmix_BP`, `Req_Qty`, `Remain_Qty`, `Post_Qty`, `BP`, `CreateByERP`, `CreateDateERP`, `UpdateByERP`, `UpdateDateERP`, CreateByBP, CreateDateBP, Regional,
								`Material`, `Keterangan`, `Index_Load`) VALUES ('live-bspi', 'POST', '" . $jo['JO_Number'] . "', '" . $jo['Item_Code'] . "', '" . $load['Item_Code'] . "', " . $jo['Item_Qty'] . ", " . ($jo['Remaining_Qty'] - $total_qty_load) . ", " . $load['Load_Size'] . ", '" . $load['bp_name'] . "', '" . $jo['Created_By'] . "', 
					...
					$query = "	INSERT INTO `API_Post_Gagal`(`type`, `JO_Number`, `Jobmix_ERP`, `Jobmix_BP`, `Index_Load`, `Load_Size`, `Material`, `Keterangan`, `RecordDate`, `jml_post`) 
									VALUES ('live-bspi', '" . $jo['JO_Number'] . "', '" . $jo['Item_Code'] . "', '" . $load['Item_Code'] . "', " . $load['index_load'] . ", " . $load['Load_Size'] . ", '" . $material . "', '" . $result['return'] . "', '" . $RecordDate->format("Y-m-d H:i:s") . "', 1)";
					mysqli_query($conmysql, $query);
				}
				sleep(1);
			} else {
				...
				$query = "	INSERT INTO `API_Post_Gagal`(`type`, `JO_Number`, `Jobmix_ERP`, `Jobmix_BP`, `Index_Load`, `Load_Size`, `Material`, `Keterangan`, `RecordDate`, `jml_post`) 
								VALUES ('live-bspi', '" . $jo['JO_Number'] . "', '" . $jo['Item_Code'] . "', '" . $load['Item_Code'] . "', " . $load['index_load'] . ", " . $load['Load_Size'] . ", '" . $material . "', '" . $invalid . "', '" . $RecordDate->format("Y-m-d H:i:s") . "', 1)";
				mysqli_query($conmysql, $query);
			}
		}
		// }
		...
		if (mysqli_query($conmysql, $query)) {
			echo $jo['JO_Number'] . " SUKSES ditambahkan ke Temp_API<br>===================================<b class='blue'>HEADER</b>==<b class='blue'>SUKSES</b>=============================================================<br>";
		} else {
			echo $jo['JO_Number'] . " GAGAL insert ke Temp_API<br>=====================================<b class='red'>HEADER</b>==<b class='red'>GAGAL</b>============================================================<br>";
		}
