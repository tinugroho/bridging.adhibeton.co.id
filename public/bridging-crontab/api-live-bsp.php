...
class Api
{
	...
	public function last_post_id($JO_Number)
	{
		...
		$query = "	select Index_Load from API_Logs_Detail where Method = 'POST' and JO_Number = '" . $JO_Number . "' and `type`='live-bsp' 
					union
					(select Index_Load from API_Post_Gagal where JO_Number = '" . $JO_Number . "' and `type`='live-bsp')
	...
	public function loadJoNumber($JO_Number)
	{
		...
		$apiupdate = " and a.RecordDate > '2021-08-02 17:00'";
		...
		$query = "select max(a.index_load) as index_load ,a.LoadID, max(a.RecordDate) as RecordDate, a.Load_Size, a.Item_Code, a.BP_ID, a.CreatedBy, a.RecordDate, a.Ticket_Code, a.Truck_Code, a.Driver_Code, b.bp_name, b.api_time_correction, c.region_name
				from `V_BatchSetupTickets` a
					...
					inner join `Region` c on b.id_region=c.id_region
				where a.Delivery_Instruction = '" . $JO_Number . "' " . $last_post_id . $apiupdate . " and a.Ticket_Status='O' and b.api='live-bsp' group by LoadID order by index_load asc limit 5";
	...
	{
		$Truck_Code = str_replace(' ', '', $Truck_Code);
		$Driver_Code = str_replace(' ', '', $Driver_Code);
		$url = 	"http://192.168.100.17/apberp/erp/eaccounting/tools/api/sfservice.cfc?method=setDataBSP&JO_Number=" . $JO_Number . "&SO_Number=" . $SO_Number . "&Reference=" . $index_load . "&Trx_Date=" . $RecordDate->format('Y-m-d') . "&Ship_Distance=1&Output_Qty=" . $Load_Size . "&Input_Qty=" . $material . "&Driver=" . $Driver_Code . "&Vehicle_No=" . $Truck_Code . "&Trx_ID=" . $Ticket_Code;
	...
	public function logGet($JO_Number, $Item_Code, $Item_Qty, $Remaining_Qty, $Created_By, $Created_Date, $Updated_By, $Last_Update)
	{
		$query = "INSERT INTO `API_Logs_Detail`(`type`, `Method`, `JO_Number`, `Jobmix_ERP`, `Req_Qty`, `Remain_Qty`, `CreateByERP`, `CreateDateERP`, `UpdateByERP`, `UpdateDateERP`, 
								`Keterangan`) VALUES ('live-bsp', 'GET', '" . $JO_Number . "', '" . $Item_Code . "', " . $Item_Qty . ", " . $Remaining_Qty . ", '" . $Created_By . "', 
	...
	public function logPost($JO_Number, $Jo_Item_Code, $Load_Item_Code, $Item_Qty, $Remaining_Qty, $total_qty_load, $Load_Size, $bp_name, $Created_By,  $Created_Date, $Updated_By, $Last_Update, $CreatedBy, $RecordDate, $region_name, $index_load, $material)
	{
		$query = "INSERT INTO `API_Logs_Detail`(`type`, `Method`, `JO_Number`, `Jobmix_ERP`, `Jobmix_BP`, `Req_Qty`, `Remain_Qty`, `Post_Qty`, `BP`, `CreateByERP`, `CreateDateERP`, `UpdateByERP`, `UpdateDateERP`, CreateByBP, CreateDateBP, Regional,
								`Material`, `Keterangan`, `Index_Load`) VALUES ('live-bsp', 'POST', '" . $JO_Number . "', '" . $Jo_Item_Code . "', '" . $Load_Item_Code . "', " . $Item_Qty . ", " . ($Remaining_Qty - $total_qty_load) . ", " . $Load_Size . ", '" . $bp_name . "', '" . $Created_By . "', 
	...
	public function logGagal($JO_Number, $Item_Code_ERP, $Item_Code_BP, $index_load, $Load_Size, $material, $SO_Number, $Ticket_Code, $Driver_Code, $Truck_Code, $keterangan, $RecordDate, $jml_post, $id_bp)
	{
		$Truck_Code = str_replace(' ', '', $Truck_Code);
		$Driver_Code = str_replace(' ', '', $Driver_Code);
		$query = "	INSERT INTO `API_Post_Gagal`(`type`, `JO_Number`, `Jobmix_ERP`, `Jobmix_BP`, `Index_Load`, `Load_Size`, `Material`, `SO_Number`, `Ticket_Code`, `Driver_Code`, `Truck_Code`,  `Keterangan`, `RecordDate`, `jml_post`,`bp_id`) 
									VALUES ('live-bsp', '" . $JO_Number . "', '" . $Item_Code_ERP . "', '" . $Item_Code_BP . "', " . $index_load . ", " . $Load_Size . ", '" . $material . "', '$SO_Number', $Ticket_Code, '$Driver_Code', '" . $Truck_Code . "', '" . $keterangan . "', '" . $RecordDate->format('Y-m-d H:i:s') . "', $jml_post, $id_bp )";
		if (mysqli_query($this->conmysql, $query)) {
	...
	public function cekbpid($index_load)
	{
		$query = "SELECT bp_id as bp_id FROM V_BatchSetupTickets where Index_Load = '" . $index_load . "' ";

		$result = mysqli_query($this->conmysql, $query);

		$data = array();
		while ($valbp = mysqli_fetch_array($result)) {
			$data[] = $valbp;
		}
...
if (!empty($databsp['databsp2'])) {
	...
	foreach ($databsp['databsp2'] as $jo) {
		...
		foreach ($arrLoadJoNumber as $load) {
			...
			// $cekbp_id = $api->cekbpid($load['LoadID']);
			$bp_id = $load['BP_ID'];
			...
			if ($material != '' && $kirim == true && empty($validasi)) {
				...
				if ($setDataBSP['return'] == 1) {
					...
					// LOG POST GAGAL
					$api->logGagal($jo['JO_Number'], $jo['Item_Code'], $load['Item_Code'], $load['index_load'],  $load['Load_Size'], $material, $jo['SO_Number'], $load['Ticket_Code'], $load['Driver_Code'], $load['Truck_Code'], $setDataBSP['return'], $RecordDate, 1, $bp_id);
				}
				...
				// LOG POST INVALID
				$api->logGagal($jo['JO_Number'], $jo['Item_Code'], $load['Item_Code'], $load['index_load'],  $load['Load_Size'], $material, $jo['SO_Number'], $load['Ticket_Code'], $load['Driver_Code'], $load['Truck_Code'], $invalid, $RecordDate, 0, $bp_id);
			}
		}
